...
async def generate_bid_task(task_id: str, rfp_data: dict[str, Any]):
    ...
    """
    logger.info(
        f"[Task {task_id}] Starting bid generation for RFP: {rfp_data.get('title')}"
    )
    task_status[task_id] = {
        "status": "running",
        "started_at": datetime.now().isoformat(),
    ...
    try:
        ...
        logger.info(
            f"[Task {task_id}] Bid generation completed: {bid_document.get('bid_id')}"
        )
...
async def calculate_pricing_task(task_id: str, rfp_data: dict[str, Any]):
    ...
    """
    logger.info(
        f"[Task {task_id}] Starting pricing calculation for RFP: {rfp_data.get('title')}"
    )
    task_status[task_id] = {
        "status": "running",
        "started_at": datetime.now().isoformat(),
