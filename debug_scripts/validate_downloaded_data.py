            ...
            try:
                if file_size_mb > 100:  # For large files, estimate
                    with open(file_path) as f:
                        ...
                        estimated_rows = int((lines_in_chunk * file_size_mb) - 1)  # -1 for header
...
def save_validation_report(validation_results, field_matches, category_findings, base_path):
    ...
    report = {
        ...
        'successful_validations': len([r for r in validation_results.values() if r['status'] == 'SUCCESS']),
    ...
    with open(summary_path, 'w') as f:
        ...
        f.write("Looking for NAICS codes: 312112 (beverages), 236/237 (construction), 484/492 (delivery)\n\n")
        for file, findings in category_findings.items():
            f.write(f"**{file}:**\n")
            f.write(f"- NIGP columns: {findings['nigp_columns'] if findings['nigp_columns'] else 'None found'}\n")
            f.write(f"- NAICS columns: {findings['naics_columns'] if findings['naics_columns'] else 'None found'}\n")
            f.write(f"- Sample categories: {findings['sample_categories'] if findings['sample_categories'] else 'None in sample'}\n\n")
        f.write("## Next Steps\n\n")
