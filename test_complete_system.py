...
def test_complete_bid_generation_system():
    ...
    try:
        ...
        doc_generator = BidDocumentGenerator(
            compliance_generator=compliance_gen, pricing_engine=pricing_engine
        )
        ...
        for i, test_rfp in enumerate(test_rfps, 1):
            ...
            try:
                ...
                if decision_result.recommendation in ["go", "review"]:
                    print(
                        "📝 Step 2: Generating bid document (GO/REVIEW recommendation)..."
                    )
                    doc_start = time.time()
                    bid_document = doc_generator.generate_bid_document(test_rfp)
                    # Export bid document
                    md_path = doc_generator.export_bid_document(
                        bid_document, "markdown"
                    )
                    ...
                    print(
                        f"   Content: {bid_document['metadata']['document_stats']['content_length']:,} characters"
                    )
                    print(
                        f"   Requirements: {bid_document['metadata']['document_stats']['requirements_addressed']}"
                    )
                    print("   Exports: MD, JSON")
                else:
                    print(
                        "⏭️  Step 2: Skipping document generation (NO-GO recommendation)"
                    )
                # Step 3: Export decision analysis
                print("📊 Step 3: Exporting decision analysis...")
                decision_export = decision_engine.export_decision_analysis(
                    test_rfp, decision_result
                )
                ...
                print(
                    f"   Document: {'Generated' if document_generated else 'Skipped'}"
                )
        ...
        if system_results:
            ...
            go_decisions = len(
                [r for r in system_results if r["recommendation"] == "go"]
            )
            review_decisions = len(
                [r for r in system_results if r["recommendation"] == "review"]
            )
            nogo_decisions = len(
                [r for r in system_results if r["recommendation"] == "no_go"]
            )
            documents_generated = len(
                [r for r in system_results if r["document_generated"]]
            )
            avg_processing_time = sum(
                r["processing_time"] for r in system_results
            ) / len(system_results)
            avg_overall_score = sum(r["overall_score"] for r in system_results) / len(
                system_results
            )
            avg_confidence = sum(r["confidence"] for r in system_results) / len(
                system_results
            )
            print(
                f"✅ Successfully processed: {successful_analyses}/{len(test_rfps)} RFPs"
            )
            ...
            for result in system_results:
                print(
                    f"   RFP {result['rfp_id']}: {result['recommendation'].upper()} "
            ...
            validation_criteria = {
                ...
                "varied_decisions": len(
                    set(r["recommendation"] for r in system_results)
                )
                > 1,
                "reasonable_processing": avg_processing_time < 300,  # Under 5 minutes
                "adequate_confidence": avg_confidence >= 50,
                "appropriate_document_generation": documents_generated
                <= go_decisions + review_decisions,
                "exports_successful": all(
                    os.path.exists(r["decision_export"]) for r in system_results
                ),
            }
            print("\n🎯 SYSTEM VALIDATION:")
            passed_criteria = 0
