...
def evaluate_go_nogo(triaged_df: pd.DataFrame) -> pd.DataFrame:
...
def export_outputs(df: pd.DataFrame, file_tag="discovered_rfps_sample"):
    ...
    meta = {"discovery_timestamp": datetime.now().isoformat(), "num_rfps": len(df)}
    # JSON
    output_json = {"metadata": meta, "rfps": df.to_dict(orient="records")}
...
if __name__ == "__main__":
    ...
    df_all = pd.read_parquet(
        str(PathConfig.PROCESSED_DATA_DIR / "rfp_master_dataset.parquet")
    )
