...
class RAGValidator:
        ...
        for sector, queries in sector_queries.items():
            ...
            for query in queries:
                ...
                if retrieved_docs:
                    ...
                    if (
                        top_result["metadata"]["category"] == sector
                        or sector == "general"
                    ):
                        category_matches += 1
            ...
            sector_results["average_relevance_score"] = (
                sum(sector_results["relevance_scores"])
                / len(sector_results["relevance_scores"])
                if sector_results["relevance_scores"]
                else 0
            )
            results[sector] = sector_results
            print(
                f"  Average retrieval time: {sector_results['average_retrieval_time']:.3f}s"
            )
            print(f"  Category match rate: {sector_results['category_match_rate']:.1%}")
            print(
                f"  Average relevance score: {sector_results['average_relevance_score']:.3f}"
            )
        return results
    ...
    def test_performance_metrics(self) -> dict[str, Any]:
        ...
        return {
            ...
            "fast_retrieval": all(
                v["retrieval_time"] < 0.5 for v in k_performance.values()
            ),
        }
