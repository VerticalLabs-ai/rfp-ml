...
class RAGPricingIntegrator:
    ...
    def analyze_enhanced_pricing(
        ...
        **additional_params
        ...
        pricing_recommendations = self._generate_enhanced_recommendations(
            pricing_result,
            rag_analysis_result.generated_text,
            market_insights
        )
        ...
        return EnhancedPricingResult(
            ...
            historical_context=rag_analysis_result.context_used[:500] + "..." if len(rag_analysis_result.context_used) > 500 else rag_analysis_result.context_used,
    ...
    def _extract_market_insights(self, retrieved_documents: List[Dict]) -> List[str]:
        ...
        if len(datasets) > 1:
            insights.append(f"Analysis includes data from {len(datasets)} different contract categories for comprehensive market view")
        if price_patterns:
            avg_relevance = sum(price_patterns) / len(price_patterns)
            if avg_relevance > 0.7:
                insights.append("High relevance pricing data found in historical contracts")
            elif avg_relevance > 0.5:
                insights.append("Moderate relevance pricing data available for reference")
            else:
                insights.append("Limited directly relevant pricing data, general market trends applied")
        if len(retrieved_documents) >= 5:
            insights.append(f"Robust dataset with {len(retrieved_documents)} similar contracts analyzed")
        elif len(retrieved_documents) >= 3:
            insights.append(f"Moderate dataset with {len(retrieved_documents)} contracts for comparison")
        else:
            insights.append("Limited historical data - recommendations based on cost models and general market trends")
        return insights
    def _analyze_competitive_landscape(
        ...
        if pricing_result.margin_achieved > 0.3:
            competitive_intel["competitive_advantages"].append("Strong margin allows for service quality investment")
        if pricing_result.confidence_score > 0.7:
            competitive_intel["competitive_advantages"].append("High confidence pricing based on robust market data")
        if pricing_result.risk_assessment.get("overall_risk", "medium") == "low":
            competitive_intel["competitive_advantages"].append("Low risk pricing strategy enhances bid attractiveness")
        return competitive_intel
    def _generate_enhanced_recommendations(
        ...
        if pricing_result.margin_compliance:
            recommendations.append(f"Recommended price of ${pricing_result.recommended_price:,.2f} meets margin requirements with {pricing_result.margin_achieved:.1%} margin")
        else:
            recommendations.append("WARNING: Recommended price does not meet minimum margin requirements - consider cost reduction or pricing strategy adjustment")
        # Strategy-specific recommendations
        if pricing_result.strategy_used == PricingStrategy.HYBRID:
            recommendations.append("Hybrid pricing strategy balances market competitiveness with cost coverage")
        elif pricing_result.strategy_used == PricingStrategy.MARKET_BASED:
            recommendations.append("Market-based pricing aligns with industry standards for competitive positioning")
        elif pricing_result.strategy_used == PricingStrategy.COST_PLUS:
            recommendations.append("Cost-plus pricing ensures full cost recovery with transparent margin structure")
        # Risk-based recommendations
        overall_risk = pricing_result.risk_assessment.get("overall_risk", "medium")
        if overall_risk == "high" or overall_risk == "critical":
            recommendations.append("High pricing risk detected - consider alternative pricing strategies or additional market research")
        # Market data recommendations
        if pricing_result.market_analysis.similar_contracts_count < 3:
            recommendations.append("Limited market data available - monitor competitor responses and adjust pricing as needed")
        # Confidence-based recommendations
        if pricing_result.confidence_score < 0.5:
            recommendations.append("Lower confidence in pricing due to limited data - consider conservative margin approach")
        elif pricing_result.confidence_score > 0.8:
            recommendations.append("High confidence pricing - well-supported by market data and cost analysis")
        # RAG-enhanced recommendations
        if "competitive" in rag_analysis.lower():
            recommendations.append("Historical data suggests competitive market - pricing strategy should emphasize value proposition")
        if "premium" in rag_analysis.lower():
            recommendations.append("Market analysis indicates potential for premium pricing based on quality differentiation")
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"✓ Enhanced Capabilities: {list(status['enhanced_capabilities'].keys())}")
        ...
        print(f"✓ Enhanced Price: ${enhanced_result.pricing_result.recommended_price:,.2f}")
        print(f"✓ Base Confidence: {enhanced_result.pricing_result.confidence_score:.2f}")
        print(f"✓ Confidence Enhancement: +{enhanced_result.confidence_enhancement:.2f}")
